...
int obtener_tabla_de_nivel_2(uint32_t nroDeTabla1, uint32_t entradaDeTabla1, int cantidadProcesosMax, t_tabla_nivel_1* tablasDeNivel1) {
    for (int i = 0; i < cantidadProcesosMax; i++) {
...
uint32_t obtener_tabla_de_nivel_1(uint32_t nroDeTabla2, int cantidadProcesosMax, int entradasPorTabla, t_tabla_nivel_1* tablasDeNivel1) {
    for (int i = 0; i < cantidadProcesosMax; i++) {
        for (int j = 0; j < entradasPorTabla; j++) {
...
uint32_t obtener_tabla_libre_de_nivel_1(int cantidadProcesosMax, t_tabla_nivel_1* tablasDeNivel1) {
    int i;
...
uint32_t asignar_tabla_nivel_1(int indiceTablaNivel1, uint32_t tamanio, t_tabla_nivel_1* tablasDeNivel1, int contadorTabla1) {
    tablasDeNivel1[indiceTablaNivel1].tamanio = tamanio;
    tablasDeNivel1[indiceTablaNivel1].id = contadorTabla1++;  // TODO mutex?
    return tablasDeNivel1[indiceTablaNivel1].id;
}

int obtener_marco_libre(uint32_t nroTablaNivel1, int cantidadMarcosProceso, t_tabla_nivel_1* tablasDeNivel1) {
    int indiceTablaNivel1 = obtener_indice_tabla_nivel_1(nroTablaNivel1, cantidadMarcosProceso, tablasDeNivel1);
    for (int i = 0; i < cantidadMarcosProceso; i++) {
        int marco = tablasDeNivel1[indiceTablaNivel1].marcos[i];
        if (esta_libre(marco)) {
            return marco;
        }
    }
...
uint32_t obtener_indice_tabla_nivel_1(uint32_t nroTablaNivel1, int cantidadProcesosMax, t_tabla_nivel_1* tablasDeNivel1) {
    for (int i = 0; i < cantidadProcesosMax; i++) {
