...
static uint32_t __cpu_solicitar_a_memoria(int toSocket, uint32_t numeroDeTabla, uint32_t entradaDeTabla, t_header requestHeader) {
...
static uint32_t __cpu_obtener_marco(t_tlb *tlb, int toSocket, uint32_t direccion, uint32_t numeroDeTabla1) {
    ...
    if (marco < 0) {
        uint32_t numeroDeTabla2 = __cpu_solicitar_a_memoria(toSocket, numeroDeTabla1, entradaTablaNivel1, HEADER_tabla_nivel_2);
        marco = __cpu_solicitar_a_memoria(toSocket, numeroDeTabla2, entradaTablaNivel2, HEADER_marco);
        tlb_registrar_entrada_en_tlb(tlb, numeroPagina, marco);
    }
    return marco + offset;
}

void cpu_escribir_en_memoria(t_tlb *tlb, int toSocket, uint32_t direccionAEscribir, uint32_t numberoDeTabla1, uint32_t contenidoAEscribir) {
    int marco = __cpu_obtener_marco(tlb, toSocket, direccionAEscribir, numberoDeTabla1);
...
uint32_t cpu_leer_en_memoria(t_tlb *tlb, int toSocket, uint32_t direccionALeer, uint32_t numberoDeTabla1) {
    int marco = __cpu_obtener_marco(tlb, toSocket, direccionALeer, numberoDeTabla1);
    t_buffer *requestBuffer = buffer_create();
